...
class Movements:
    ...
    def __init__(
        self,
        name,
        speed_enable="SpeedEnable",
        attitude_enable="AttitudeEnable",
        turn_enable="TurnEnable",
        legs_enable="LegsEnable",
    ):
    ...
    def getCycleTicks(self):
        ...
        Speedticks = (
            ...
            * self.SpeedMovements.ExecuteCounterMax
        )
        Attitudeticks = (
            self.AttitudeMovements.PhaseNumberMax
            * self.AttitudeMovements.InterpolationNumber
            * self.AttitudeMovements.ExecuteCounterMax
        )
        Legsticks = (
            self.LegsMovements[0].PhaseNumberMax
            * self.LegsMovements[0].InterpolationNumber
            * self.LegsMovements[0].ExecuteCounterMax
        )
        Turnticks = (
            self.TurnMovements.PhaseNumberMax
            * self.TurnMovements.InterpolationNumber
            * self.TurnMovements.ExecuteCounterMax
        )
...
class MovementScheme:
    ...
    def updateMovement(self, movement_type):
        ...
        if movement_type != self.movement_now_name:
            ...
            self.entry_down
            and self.entry_down1
            and self.entry_down2
            and self.entry_down3
        ):
        ...
        if self.ststus == "Entry":
            ...
            if self.getAccCommand == True:
                ...
                self.movements_now.DeltLegsM = self.movements_now.DeltLegsM.astype(
                    np.float64
                )
                self.movements_now.DeltLegsM /= self.movements_now.transTic

                self.movements_now.DeltSpeedM = speed_ready - self.speed_pre
                self.movements_now.DeltSpeedM = self.movements_now.DeltSpeedM.astype(
                    np.float64
                )
                ...
                self.movements_now.DeltAttitudeM = (
                    self.movements_now.DeltAttitudeM.astype(np.float64)
                )
                self.movements_now.DeltAttitudeM /= self.movements_now.transTic

                self.movements_now.DeltTurnM = turn_ready - self.turn_pre
                self.movements_now.DeltTurnM = self.movements_now.DeltTurnM.astype(
                    np.float64
                )
                self.movements_now.DeltTurnM /= self.movements_now.transTic
                self.getAccCommand = False

            self.legs_location_now, self.entry_down = (
                self.updateMovementLegsLocationGradient(
                    self.legs_location_pre, location_ready
                )
            )
            self.legs_location_pre = self.legs_location_now

            self.speed_now, self.entry_down1 = self.updateMovementSpeedGradient(
                self.speed_pre, speed_ready
            )
            self.speed_pre = self.speed_now

            self.attitude_now, self.entry_down2 = self.updateMovementAttitudeGradient(
                self.attitude_pre, attitude_ready
            )
            self.attitude_pre = self.attitude_now

            self.turn_now, self.entry_down3 = self.updateMovementTurnGradient(
                self.turn_pre, turn_ready
            )
            self.turn_pre = self.turn_now

            if (
                self.entry_down == True
                and self.entry_down1 == True
                and self.entry_down2 == True
                and self.entry_down3 == True
            ):
        ...
        if self.ststus == "Exit":
            if self.movements_pre.ExitToStand == True:
                ...
                self.legs_location_now, self.exit_down = (
                    self.updateMovementLegsLocationGradient(
                        self.legs_location_pre, LocationStanding
                    )
                )
                ...
                # update the speed
                self.speed_now, self.exit_down1 = self.updateMovementSpeedGradient(
                    self.speed_pre, SpeedStanding
                )
                self.legs_location_pre = self.legs_location_now

                # update the attitude
                self.attitude_now, self.exit_down2 = (
                    self.updateMovementAttitudeGradient(
                        self.attitude_pre, AttitudeStanding
                    )
                )
                self.attitude_pre = self.attitude_now
                ...
                self.turn_now, self.exit_down3 = self.updateMovementAttitudeGradient(
                    self.turn_pre, 0
                )
                self.turn_pre = self.turn_now
                ...
                if (
                    self.exit_down == True
                    and self.exit_down1 == True
                    and self.exit_down2 == True
                    and self.exit_down3
                ):
            ...
            self.tick += 1
            if (
                self.tick >= self.now_ticks
                and self.movement_now_number < len(self.movements_lib) - 1
            ):
        ...
        for xy_index in range(2):
            ...
            if abs(diff) > abs(self.movements_now.DeltSpeedM[xy_index]):
                speed_gradient[xy_index] = (
                    speed_now[xy_index] + self.movements_now.DeltSpeedM[xy_index]
                )
            else:
